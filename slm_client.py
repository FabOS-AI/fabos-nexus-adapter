...
class slmClient():
    ...
    def update_service_offering_version(self, offering_version: any) -> object:
        """Gets the resource for the given uuid at resource registry
        ...
        res = requests.put(
            url=f"{self.host_service_registry}/services/offerings/{offering_version['serviceOfferingId']}/versions/{offering_version['id']}",
            data=json.dumps(offering_version),
            headers=headers
        )

        if res.status_code in [200, 201]:
            print(f"SUCCESS({res.status_code}): found offering version '{offering_version['serviceOfferingId']}/{offering_version['id']}' in registry")
            return res.json()
        else:
            print(f"FAILED({res.status_code}): could not update offering version '{offering_version['serviceOfferingId']}/{offering_version['id']}' in registry")
            return {}
        
    def get_service_offering_version(self, offering_id: str, offering_version_id: str) -> object:
        """Gets the service offering version for the IDs at service registry
        """
        ...
        res = requests.get(
            url=f"{self.host_service_registry}/services/offerings/{offering_id}/versions/{offering_version_id}",
            headers=headers
        )

        if res.status_code in [200, 201]:
            print(f"SUCCESS({res.status_code}): found offering version '{offering_id}/{offering_version_id}' in registry")
            return res.json()
        else:
            print(f"FAILED({res.status_code}): could not found offering version '{offering_id}/{offering_version_id}' in registry")
            return {}